...
def get_metric_realtime(services, minutes=5):
    ...
    for service in services:
        ...
        # QUERY TEMPO
        #
        network_p90 = get_network_latency(service, minutes)
        ...
        try:
            cpu_result = metrics["cpu"]["data"]["result"]

            if len(cpu_result) > 0:
                cpu_value = float(cpu_result[0]["values"][-1][1])
        ...
        try:
            memory_result = metrics["memory"]["data"]["result"]

            if len(memory_result) > 0:
                memory_value = float(memory_result[0]["values"][-1][1])

        except:
            pass

        #
        # BUILD SAME FORMAT AS OFFLINE
        #
        target_metric = {
            "pod": service,
            "metrics": [
